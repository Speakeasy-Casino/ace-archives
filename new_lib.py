...
def train_vailidate_test_split(df, target, strat = None):
    ...
    '''
    if strat:
        train_validate, test = train_test_split(df, train_size =.5, random_state = 91, stratify = df[target])
        train, validate = train_test_split(train_validate, train_size = .6, random_state = 91, stratify = train_validate[target])
    else:
